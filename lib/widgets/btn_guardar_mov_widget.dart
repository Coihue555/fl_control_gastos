  ...
  Widget build(BuildContext context) {
    return ElevatedButton(
        ...
        onPressed: () {
          ...
          if (dropdownCategoria.isEmpty) {
            dropdownCategoria = movState.categoria;
          }
          if (dropdownCuenta.isEmpty) {
            dropdownCuenta = movState.cuenta;
          }
          if (dateinput.text.isEmpty) {
            dateinput.text = movState.fecha;
          }
          if (spDescripcion.isEmpty) {
            spDescripcion = movState.descripcion;
          }
          if (spValor == 0.0) {
            spValor = movState.valor;
          }

          context.read<MovimientosBloc>().add(ValidateMovimiento(dropdownCategoria, dropdownCuenta, dateinput.text, spDescripcion, spValor));
        });
  }
}
