...
class CuentaFichaScreen extends StatefulWidget {
  const CuentaFichaScreen({Key? key}) : super(key: key);
...
class _CuentaFichaScreenState extends State<CuentaFichaScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<CuentasBloc, CuentasState>(builder: (context, state) {
      return Container(
    ...
    return ElevatedButton(
        ...
        onPressed: () {
          if (nombreCuenta.isEmpty) {
            nombreCuenta = state.cuenta.nombreCuenta;
          }
          context.read<CuentasBloc>().add(ValidateCuenta(nombreCuenta));
        });
  }
}
